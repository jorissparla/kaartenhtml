    ...
    <div className="bg-slate-900 min-h-screen text-white">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="ml-4 text-sm bg-slate-700 hover:bg-slate-600 py-2 px-3 rounded transition-colors"
          >
            Settings
          </button>
        </div>
        ...
        <div className="mb-16">
          <div className="max-w-md mx-auto bg-slate-800 p-6 rounded-lg shadow-lg mb-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold">Players ({players.length})</h2>
              <button
                onClick={quickFill}
                className="text-sm bg-purple-600 hover:bg-purple-700 py-1 px-3 rounded transition-colors"
              >
            ...
            <form onSubmit={handleAddPlayer} className="flex gap-2 mb-4">
              <input
                ...
                className="flex-grow p-2 rounded bg-slate-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Enter player name"
              />
              <button type="submit" className="bg-blue-600 hover:bg-blue-700 py-2 px-4 rounded-lg text-white font-semibold transition-colors">
                Add
              </button>
            </form>
            ...
            {players.length > 0 && (
              <button
                ...
                className="w-full bg-green-600 hover:bg-green-700 py-2 px-4 rounded transition-colors mt-4"
              >
          ...
          {showResults && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8">
                {cards.map((c) => (
                  <div key={`${c.name}-${c.number}`} className="bg-slate-800 p-6 rounded-lg shadow-lg text-center">
                    <h3 className="text-xl font-bold mb-2">{c.name}</h3>
                    <div className="text-3xl font-bold text-blue-500">{c.number}</div>
                  </div>
                ))}
              </div>

              <hr className="border-t border-slate-700 my-12" />

              <div className="bg-white rounded-xl p-8">
                <div>
                  <h2 className="text-2xl font-bold mb-6 text-slate-800">Match Schedule</h2>
                  <div className="space-y-8">
                    {rounds.map((round, roundIndex) => (
                      <div key={roundIndex} className="bg-slate-100 p-6 rounded-lg shadow-sm">
                        <h3 className="text-xl font-bold mb-4 text-slate-800">Round {roundIndex + 1}</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {round.map((match, i) => (
                            <div
                              key={i}
                              className="bg-white p-4 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"
                            >
                              <div className="text-slate-600 text-sm font-semibold mb-3 border-b border-slate-200 pb-2">
                                Court {i + 1}
                              </div>
                              <div className="flex items-center justify-between gap-4">
                                <div className="w-5/12">
                                  <div className="flex items-center gap-2 mb-2">
                                    <span className="inline-flex items-center justify-center bg-blue-600 text-white w-6 h-6 rounded-full text-xs font-bold">
                                      {match.team1[0].number}
                                    </span>
                                    <span className="text-slate-700 truncate">{match.team1[0].name}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="inline-flex items-center justify-center bg-blue-600 text-white w-6 h-6 rounded-full text-xs font-bold">
                                      {match.team1[1].number}
                                    </span>
                                    <span className="text-slate-700 truncate">{match.team1[1].name}</span>
                                  </div>
                                </div>
                                <div className="text-sm font-bold text-slate-400">VS</div>
                                <div className="w-5/12 text-right">
                                  <div className="flex items-center gap-2 justify-end mb-2">
                                    <span className="text-slate-700 truncate">{match.team2[0].name}</span>
                                    <span className="inline-flex items-center justify-center bg-blue-600 text-white w-6 h-6 rounded-full text-xs font-bold">
                                      {match.team2[0].number}
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-2 justify-end">
                                    <span className="text-slate-700 truncate">{match.team2[1].name}</span>
                                    <span className="inline-flex items-center justify-center bg-blue-600 text-white w-6 h-6 rounded-full text-xs font-bold">
      ...
      {settingsOpen && (
        <div className={`fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center p-4 z-50 ${settingsOpen ? 'settings-sheet-open' : ''}`} onClick={closeSettings}>
          <div
            className="settings-sheet bg-slate-800 w-full max-w-xl rounded-t-lg sm:rounded-lg shadow-xl transform transition-transform duration-300 ease-in-out translate-y-full sm:translate-y-0"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-slate-700">
              <h3 className="text-lg font-semibold">Settings — Sample Names</h3>
              <button onClick={closeSettings} className="text-slate-300 hover:text-white">
            ...
            <div className="p-4 space-y-4">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 py-2 px-4 rounded-lg text-white font-semibold transition-colors"
                >
                  Add
                </button>
              </div>
              <div className="h-60 overflow-y-auto border border-slate-700 rounded-lg">
            ...
            <div className="p-4 flex items-center justify-end gap-4 border-t border-slate-700">
              <button onClick={closeSettings} className="bg-slate-700 hover:bg-slate-600 py-2 px-4 rounded-lg text-white font-semibold transition-colors">
                Cancel
              </button>
              <button onClick={saveSettings} className="bg-blue-600 hover:bg-blue-700 py-2 px-4 rounded-lg text-white font-semibold transition-colors">
                Save Changes
              </button>
            </div>
          </div>
